...
# Carga de plantilla de evaluación desde Excel (archivo en el mismo directorio)
raw = pd.read_excel('Plan de Calidad emitida SegurCaixa eCommerce.xlsx', header=4)
titles = raw.iloc[0]
raw = raw.iloc[1:].reset_index(drop=True)
raw = raw.rename(columns={col: titles[col] for col in raw.columns})
# Seleccionar columnas relevantes y limpiar
df_templates = raw[["Bloque","Ítem","Pauta","Descripción","Respuestas","PENC","PECUF","PECC","PECN"]]
df_templates = df_templates[df_templates['Ítem'].notna()].reset_index(drop=True)
...
df_trend = (
    pd.to_datetime(df_detail['Fecha/Hora'], dayfirst=True)
