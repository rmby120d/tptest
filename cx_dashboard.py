...
with tabs[2]:
    ...
    ax_wc.imshow(wc.recolor(color_func=lambda *args,**kwargs:COLOR_CORP), interpolation='bilinear')
    ax_wc.axis('off')
    st.pyplot(fig_wc)
...
with tabs[3]:
    ...
    st.markdown("---")
    # 4. Eliminado Parallel Coordinates y Sankey por errores
...
with tabs[5]:
    ...
    fig_sent = px.bar(
        cnt, x='Sentimiento', y='Conteo', color='Rol', barmode='group',
